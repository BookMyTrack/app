...
const TrackCard: FC<PropsWithChildren<ITrackCardProps>> = ({ track }) => {
	// const event = useMemo(
	// 	() => track.events.sort((a, b) => compareDesc(new Date(a.start), new Date(b.start)))?.[0],
	// 	[track]
	// );

	// const days = useMemo(() => differenceInDays(new Date(event?.start ?? Date.now()), new Date()), [event]);
	...
	return (
		<div
			...
			style={{ background: `url(${track.image_url ?? `/track-photos/${track.id}.jpeg`}) no-repeat center`, backgroundSize: 'cover' }}
		>
			<div className={clsx('to-black/25 from-black bg-gradient-to-t w-full h-full overflow-hidden')}>
				<div className="flex flex-col items-start justify-end w-full h-full">
					<div className="px-8 mx-auto">
						<img alt={track.name} src={`/track-images/${track.id}.svg`} />
					</div>
					...
					<div className="bg-black/25 backdrop-blur-sm flex items-center justify-end w-full gap-3 p-4 text-white/80">
						{days === 0 ? (
							...
							<>
								Next event in {days ?? '--'} days <CalendarIcon className="w-5" />
							</>
						)}
					</div>
				</div>
