import { CalendarIcon, CurrencyDollarIcon, LocationMarkerIcon, MapIcon } from '@heroicons/react/outline';
...
const TrackHeader: FC<PropsWithChildren<ITrackDaysHeaderProps>> = ( { track } ) => {
	...
	const datesRange = useMemo( () => {
		const dates = track.events.map( track => new Date( track.start ) ).sort( ( a, b ) => a.getTime() - b.getTime() );

		if ( dates.length === 0 ) return '--';

		if ( dates.length === 1 ) return format( dates[0], 'MMM' );
		return `${format( dates[0], 'MMM' )} - ${format( dates[dates.length - 1], 'MMM' )}`;
	}, [track.events] );

	return (
		<div className="lg:flex lg:items-center lg:justify-between rx-text-neutral-11 py-4">
			<div className="flex-1 min-w-0">
				<h2 className="sm:text-3xl sm:truncate rx-text-neutral-12 text-2xl font-bold leading-7">{track.name}</h2>
				<div className="sm:flex-row sm:flex-wrap sm:mt-0 sm:space-x-6 flex flex-col mt-1">
					<div className="flex items-center mt-2 text-sm">
						<LocationMarkerIcon className="flex-shrink-0 mr-1.5 h-5 w-5" aria-hidden="true" />
						{track.address}
					</div>
					<div className="flex items-center mt-2 text-sm">
						<CurrencyDollarIcon className="flex-shrink-0 mr-1.5 h-5 w-5" aria-hidden="true" />
						{priceRange}
					</div>
